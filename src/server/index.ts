...
const cleanupOrphanedApiConfigs = () => {
  try {
    ...
    const usedApiConfigIds = new Set(
      allServers.map((s: any) => s.api_config_id).filter(Boolean),
    );
    ...
    allApiConfigs.forEach((config: any) => {
      if (!usedApiConfigIds.has(config.id)) {
        ...
        cleanedCount++;
        console.log(
          `Cleaned up orphaned API config: ${config.id} (${config.name})`,
        );
      }
    });
...
app.listen(PORT, () => {
  console.log(
    `🚀 UNITONE MCP Generator API running on http://localhost:${PORT}`,
  );
