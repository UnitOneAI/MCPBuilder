...
export const apiService = {
  ...
  // Deployments
  getDeployment: (serverId: string) =>
    api.get(`/deployments/server/${serverId}`),
  deployServer: (serverId: string) =>
    api.post(`/deployments/${serverId}/deploy`),
  stopServer: (serverId: string) => api.post(`/deployments/${serverId}/stop`),

  // Parser
  parseOpenApi: (spec: string) => api.post("/parser/openapi", { spec }),
  parseDocumentation: (documentation: string) =>
    api.post("/parser/documentation", { documentation }),
  parsePostman: (collection: string) =>
    api.post("/parser/postman", { collection }),
};
