...
function StepGenerate({ serverId, isGenerated = false }: StepGenerateProps) {
  ...
  const serverPath =
    server?.generated_path ||
    (server?.name
      ? `<YOUR_PROJECT_PATH>/generated-servers/${server.name.toLowerCase().replace(/[^a-z0-9-]/g, "-")}`
      : "<YOUR_PROJECT_PATH>/generated-servers/<server-name>");
      ...
      <Box py={4}>
        <Box textAlign="center" mb={4}>
          ...
          <Typography variant="body1" color="textSecondary" paragraph>
            Your server configuration is complete. Click "Generate" below to
            create your MCP server code.
          </Typography>
        </Box>
        ...
        <Alert severity="info" sx={{ mb: 3 }}>
          ...
          <ul style={{ marginLeft: 20, marginBottom: 0 }}>
            <li>
              <strong>Server Code:</strong> TypeScript MCP server implementation
            </li>
            <li>
              <strong>Tool Definitions:</strong> {server?.tools?.length || 0}{" "}
            ...
            <li>
              ...
              {server?.transport === "stdio"
                ...
                : `HTTP on port ${server?.httpConfig?.port || 3000}`}
            </li>
            <li>
              <strong>Dependencies:</strong> package.json with all required
              packages
            </li>
            <li>
              <strong>Configuration:</strong> Ready-to-use Claude Desktop config
            </li>
          </ul>
        </Alert>
        ...
        <Paper sx={{ p: 3, bgcolor: "grey.50" }}>
          ...
          >
            ...
            <Typography variant="body2">
              {server?.api_base_url || "(Set via environment variable)"}
            </Typography>
            ...
            <Typography variant="body2">
              {server?.tools?.length || 0} endpoints
            </Typography>
          </Box>
        </Paper>
      </Box>
  ...
  const getClaudeConfig = () => {
    ...
    if (server.transport === "stdio") {
      ...
      if (
        server.api_auth_type === "bearer" ||
        server.api_auth_type === "oauth2"
      ) {
    ...
    <Box py={4}>
      ...
      <Alert severity="warning" sx={{ mb: 3 }}>
        ...
        <Typography variant="body2" component="div">
          Before restarting Claude Desktop, make sure to replace the placeholder
          values in the configuration above:
        </Typography>
        <ul style={{ marginTop: 8, marginBottom: 0, fontSize: "0.875rem" }}>
          <li>
            <code>YOUR_API_BASE_URL</code> - Replace with the actual API base
            URL
          </li>
          {server?.api_auth_type === "apiKey" && (
            <li>
              <code>YOUR_API_KEY_HERE</code> - Replace with your actual API key
            </li>
          )}
          {(server?.api_auth_type === "bearer" ||
            server?.api_auth_type === "oauth2") && (
            <li>
              <code>YOUR_API_TOKEN_HERE</code> - Replace with your actual API
              token
            </li>
          )}
        </ul>
      ...
      <Alert severity="success" sx={{ mb: 3 }}>
        ...
        <ol style={{ marginLeft: 20, marginBottom: 0 }}>
          ...
          <li>
            Replace the placeholder values (API_BASE_URL, API_KEY, etc.) with
