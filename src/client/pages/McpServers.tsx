...
function McpServers() {
  ...
  const buildMutation = useMutation({
    ...
    onSuccess: (response) => {
      ...
      enqueueSnackbar(
        response.data?.message || "STDIO server built successfully",
        { variant: "success" },
      );
    },
    onError: (error: any) => {
      enqueueSnackbar(error.response?.data?.error || "Failed to build server", {
        variant: "error",
      });
    },
  });
  ...
  const rebuildMutation = useMutation({
    ...
    onError: (error: any) => {
      enqueueSnackbar(
        error.response?.data?.error || "Failed to rebuild server",
        { variant: "error" },
      );
    },
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: (response) => {
      queryClient.invalidateQueries({ queryKey: ["mcp-servers"] });
      enqueueSnackbar(response.data?.message || "Server deleted successfully", {
        variant: "success",
      });
    },
    onError: (error: any) => {
  ...
  return (
    <Box>
      ...
      <Card sx={{ mb: 3 }}>
        <CardContent>
          ...
          {searchTerm && (
            ...
            >
              Found {filteredServers.length} server
                  ...
                  <Card>
                    <CardContent>
                      ...
                      <Box
                        ...
                        alignItems="center"
                      >
                        <Chip label="STDIO" size="small" variant="outlined" />
                        {server.api_name && (
                        ...
                        <Chip
                          ...
                          deleteIcon={
                            <IconButton
                              size="small"
                              sx={{
                                transform: expandedTools[server.id]
                                  ? "rotate(180deg)"
                                  : "rotate(0deg)",
                          ...
                          sx={{
                            cursor:
                              server.tools?.length > 0 ? "pointer" : "default",
                          }}
                        />
                      </Box>
                        ...
                        <Box display="flex" gap={1}>
                          {server.status === "draft" && (
                            <Button
                              ...
                              startIcon={<CodeIcon />}
                              onClick={() =>
                                setGenerateDialog({ open: true, server })
                              }
                            >
                              Generate Code
                            </Button>
                            ...
                            <>
                              <Tooltip
                                title={
                                  server.deployment_status === "ready"
                                    ? "Server already built - use Rebuild"
                                    : "Build Server"
                                }
                              >
                                <span>
                                  <IconButton
                                    ...
                                    color="success"
                                    onClick={() =>
                                      buildMutation.mutate(server.id)
                                    }
                                    disabled={
                                      server.deployment_status === "ready" ||
                              ...
                              <Tooltip title="Rebuild Server">
                                <IconButton
                                  ...
                                  color="primary"
                                  onClick={() =>
                                    rebuildMutation.mutate(server.id)
                                  }
                                  disabled={
                                    buildMutation.isPending ||
                                    rebuildMutation.isPending
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => {
          try {
            const response = await apiService.generateServer(
              generateDialog.server.id,
            );
            queryClient.invalidateQueries({ queryKey: ["mcp-servers"] });
            setGenerateDialog({ open: false, server: null });
            enqueueSnackbar(
              response.data?.message || "Server generated successfully",
              { variant: "success" },
            );
          } catch (error: any) {
            enqueueSnackbar(
