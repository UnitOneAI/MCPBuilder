...
export class McpGenerator {
  ...
  async generate(options: GeneratorOptions): Promise<GeneratedServer> {
    ...
    const serverDir = path.join(
      outputDir,
      this.sanitizeName(serverConfig.name),
    );
    ...
    const vscodeFiles = await this.generateVsCodeConfig(
      serverDir,
      serverConfig,
      apiConfig,
    );
  ...
  private async generateTypesFile(
    ...
    const endpointsWithDeduplicatedParams = (apiConfig.endpoints || []).map(
      (endpoint) => ({
        ...endpoint,
  ...
  private abbreviateToolName(name: string): string {
    ...
    if (abbreviated.length > 64) {
      abbreviated =
        abbreviated.substring(0, 30) +
        "_" +
        abbreviated.substring(abbreviated.length - 30);
    }
