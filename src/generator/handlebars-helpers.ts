...
export function registerHelpers() {
  ...
  Handlebars.registerHelper('mapTypeEnhanced', (param: any) => {
    ...
    if (param.enum && Array.isArray(param.enum) && param.enum.length > 0) {
      ...
      return new Handlebars.SafeString(param.enum.map((v: any) => JSON.stringify(v)).join(' | '));
    }
  ...
  Handlebars.registerHelper('hasEnum', (param: any) => {
    return param && param.enum && Array.isArray(param.enum) && param.enum.length > 0;
  });
  ...
  Handlebars.registerHelper('zodEnumWithRequired', function (this: any, enumValues: any[], fieldName: string, requiredArray: any) {
    if (!Array.isArray(enumValues) || enumValues.length === 0) {
      const isRequired = Array.isArray(requiredArray) && requiredArray.includes(fieldName);
  ...
  Handlebars.registerHelper('isRequired', function (this: any, fieldName: string, requiredArray: any) {
  ...
  Handlebars.registerHelper('zodTypeWithRequired', function (this: any, propDef: any, fieldName: string, requiredArray: any, ...args: any[]) {
    ...
    switch (lowerType) {
      ...
      case 'float':
      case 'double':
        zodType = lowerType.includes('int') ? 'z.number().int()' : 'z.number()';
